// <autogen>
import AnimationSystem, {  } from './systems/AnimationSystem';
import Engine, {  } from './Engine';
import IEngine, {  } from './interfaces/IEngine';
import IPipe, {  } from './interfaces/IPipe';
import IPipeEvent, {  } from './interfaces/IPipeEvent';
import IRenderingSystem, {  } from './interfaces/IRenderingSystem';
import IViewport, {  } from './interfaces/IViewport';
import ImageSystem, {  } from './systems/ImageSystem';
import LabelSystem, {  } from './systems/LabelSystem';
import LineSystem, {  } from './systems/LineSystem';
import Pipe, {  } from './concretes/Pipe';
import PoseSystem, {  } from './systems/PoseSystem';
import RenderingSystem, {  } from './abstracts/RenderingSystem';
import ShapeSystem, {  } from './systems/ShapeSystem';
import { RGBA_0, RGBA_WHITE, RGBA_BLACK, RGBA_GREEN, RGBA_RED, RGBA_PINK, RGBA_YELLOW } from './enums/COLOUR';
export { RenderingSystem, Pipe, RGBA_0, RGBA_WHITE, RGBA_BLACK, RGBA_GREEN, RGBA_RED, RGBA_PINK, RGBA_YELLOW, IEngine, IPipe, IPipeEvent, IRenderingSystem, IViewport, AnimationSystem, ImageSystem, LabelSystem, LineSystem, PoseSystem, ShapeSystem, Engine };
// </autogen>